...
def build_rules_brief(week_start: str, week_end: str, sources: dict) -> str:
    """No-LLM fallback: v1 Chinese brief from available digests.

    The cloud workflow can legitimately have no source files when upstream
    dispatchers failed. In that case still produce a valid data-gap report so
    the public dated URL explains the issue instead of returning 404.
    """
    facts: list[str] = []
    ...
    sources_md = "\n".join(source_list) or "- 本周窗口内未找到 `reports/digest/`、`investment-intelligence-hub/inbox/news/` 或 `reports/daily/` 源文件。"

    fact_rows: list[str] = []
    if uniq:
        for index, fact in enumerate(uniq, 1):
            fact_clean = re.sub(r"\s+", " ", strip_md(fact)).strip()
            fact_rows.append(
                f"| {index} | {week_end} | {fact_clean[:220]} | Secondary | 多卡点监控 | Neutral | DELL, VRT, AAOI, MRVL, ANET, AVGO, MU |"
            )
    else:
        fact_rows.append(
            f"| 1 | {week_end} | 本周自动化没有收集到合格的日更、新闻扫描或 digest 源文件；这是一条数据完整性缺口，不是一条基本面证伪事实。 | 内部系统状态 | 数据采集 | Neutral | 全部观察池 |"
        )
    facts_table = "\n".join(fact_rows)
    biggest_fact = (
        strip_md(uniq[0])[:180]
        if uniq
        else "本周没有可用于生成正式基本面判断的合格源文件；需要先恢复日更/新闻扫描输入。"
    )
    source_mode = "digests / news / daily" if source_list else "none - upstream data gap"
    now_text = datetime.now(NY).strftime("%Y-%m-%d %H:%M %Z")
    next_week = (datetime.fromisoformat(week_end).date() + timedelta(days=7)).isoformat()
...
**本周无 Material fact？** {"No" if uniq else "Yes — 本期无合格基本面事实；仅记录数据完整性缺口。"}
...
| 标的 | 原分类 | 新分类 | 原因（可证伪） | 下次复核 |
|---|---|---|---|---|
| — | — | — | No classification changes. | {next_week} |
...
| Author | rules_auto |
| Time spent | ~5 min |
| Sources checked | {source_mode} |
| Confidence in this week’s grade | Low — 数据源缺口，需补核 |
| Next brief due | {next_week} |
